...
interface ITreasury {
    function depositReserves(uint256 depositAmount_) external returns (bool);
}

interface ICirculatingTAO {
    function TAOCirculatingSupply() external view returns (uint256);
}

interface IBondDepo {
    function getDepositorInfo(address _depositorAddress_)
        external
        view
        returns (
            uint256 principleValue_,
            uint256 paidOut_,
            uint256 maxPayout,
    ...
    function calculatePercentVested(address depositor_)
        external
        view
        returns (uint256 _percentVested);

    function calculatePendingPayout(address depositor_)
        external
        view
        returns (uint256 _pendingPayout);

    function calculateBondInterest(uint256 value_)
        external
        view
        returns (uint256 _interestDue);

    function calculatePremium() external view returns (uint256 _premium);
}

contract TaoBUSDDepository is IBondDepo, Ownable {
    ...
    struct DepositInfo {
        uint256 value; // Value
        uint256 payoutRemaining; // TAO remaining to be paid
        uint256 lastBlock; // Last interaction
        uint256 vestingPeriod; // Blocks left to vest
    }

    mapping(address => DepositInfo) public depositorInfo;

    uint256 public DAOShare; // % = 1 / DAOShare
    uint256 public bondControlVariable; // Premium scaling variable
    uint256 public vestingPeriodInBlocks;
    uint256 public minPremium; // Floor for the premium

    //  Max a payout can be compared to the circulating supply, in hundreths. i.e. 50 = 0.5%
    uint256 public maxPayoutPercent;
    ...
    address public circulatingTAOContract; // calculates circulating supply
    ...
    constructor(
        ...
        address DAOWallet_,
        address circulatingTAOContract_
    ) {
        BUSD = BUSD_;
        TAO = TAO_;
        treasury = treasury_;
        ...
        circulatingTAOContract = circulatingTAOContract_;
    }
    ...
    function setBondTerms(
        ...
        uint256 vestingPeriodInBlocks_,
        uint256 minPremium_,
        uint256 maxPayout_,
        uint256 DAOShare_
    ...
    function _deposit(
        ...
        // Deposit token to mint TAO
        IERC20(BUSD).approve(address(treasury), amount_);
        ITreasury(treasury).depositReserves(amount_); // Returns TAO

        uint256 profit_ = value_.sub(payout_);
        uint256 DAOProfit_ = FixedPoint.fraction(profit_, DAOShare).decode();
        // Transfer profits to staking distributor and dao
        IERC20(TAO).safeTransfer(stakingContract, profit_.sub(DAOProfit_));
        ...
        depositorInfo[depositor_] = DepositInfo({
            ...
            payoutRemaining: depositorInfo[depositor_].payoutRemaining.add(
                payout_
            ),
    ...
    function redeem() external override returns (bool) {
        uint256 payoutRemaining_ = depositorInfo[msg.sender].payoutRemaining;

        require(payoutRemaining_ > 0, "Sender is not due any interest.");

        uint256 value_ = depositorInfo[msg.sender].value;
        uint256 percentVested_ = _calculatePercentVested(msg.sender);
        ...
        // calculate and send vested TAO
        uint256 payout_ = payoutRemaining_.mul(percentVested_).div(10000);
        IERC20(TAO).safeTransfer(msg.sender, payout_);

        // reduce total debt by vested amount
        uint256 valueUsed_ = value_.mul(percentVested_).div(10000);
        totalDebt = totalDebt.sub(valueUsed_);

        uint256 vestingPeriod_ = depositorInfo[msg.sender].vestingPeriod;
        uint256 blocksSinceLast_ =
            block.number.sub(depositorInfo[msg.sender].lastBlock);
    ...
    /**
        ...
        @param address_ info
     */
    function getDepositorInfo(address address_)
        external
        view
        override
        returns (
            uint256 _value,
            uint256 _payoutRemaining,
            uint256 _lastBlock,
    ...
    /**
        ...
        @return uint
     */
    function getMaxPayoutAmount() public view returns (uint256) {
        uint256 circulatingTAO =
     ...
     */
    function _calculatePercentVested(address depositor_)
        internal
        view
        returns (uint256 _percentVested)
    {
        uint256 vestingPeriod_ = depositorInfo[depositor_].vestingPeriod;
        if (vestingPeriod_ > 0) {
            uint256 blocksSinceLast_ =
                block.number.sub(depositorInfo[depositor_].lastBlock);
    ...
    /**
        @notice calculate amount of TAO available for claim by depositor
        @param depositor_ address
        @return uint
     */
    function calculatePendingPayout(address depositor_)
        ...
        returns (uint256)
    {
        uint256 percentVested_ = _calculatePercentVested(depositor_);
        uint256 payoutRemaining_ = depositorInfo[depositor_].payoutRemaining;

        uint256 pendingPayout = payoutRemaining_.mul(percentVested_).div(10000);
    ...
    function calculateBondInterest(uint256 value_)
        ...
        returns (uint256 _interestDue)
    {
        _interestDue = FixedPoint
            .fraction(value_, _calcPremium())
            .decode112with18()
            .div(1e16);
    }
    ...
    /**
        ...
        @return _premium uint
     */
    function _calcPremium() internal view returns (uint256 _premium) {
        _premium = bondControlVariable
            .mul(_calcDebtRatio())
            .add(uint256(1000000000))
            .div(1e7);
     ...
     */
    function _calcDebtRatio() internal view returns (uint256 _debtRatio) {
        uint256 supply;

        if (useCircForDebtRatio) {
            supply = ICirculatingTAO(circulatingTAOContract)
                .TAOCirculatingSupply();
        } else {
            supply = IERC20(TAO).totalSupply();
        }

        _debtRatio = FixedPoint
            .fraction(
